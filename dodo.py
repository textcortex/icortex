...
EXT_DIST_PKG = EXT / "py_src/jupyterlite_icortex/labextension/package.json"
EXT_DIST = EXT / "dist"
EXT_WHL_NAME = f"""jupyterlite_icortex-{EXT_SRC_PKG_DATA["version"]}-py3-none-any.whl"""
EXT_WHL = EXT_DIST / EXT_WHL_NAME
EXT_ICON = EXT / "style/icortex.png"
SHA256SUMS = DIST / "SHA256SUMS"
KERNEL_DATA = HERE / "icortex/kernel/icortex"
...
WHL_PY = [
    ...
    for p in [
        # HERE / "setup.py",
        *Path("icortex").rglob("*.py"),
    ]
    if p.name != "_version.py"
]
WHL_MD = [HERE / "readme.md", Path("README.md")]
WHL_DEPS = [
    HERE / "pyproject.toml",
    # HERE / "setup.cfg",
    *KERNEL_DATA.rglob("*"),
...
def task_dist():
    ...
    yield dict(
        name="icortex",
        doc="build icortex distributions",
        file_dep=WHL_DEPS,
        actions=[
            ...
            (
                ...
                [
                    SHA256SUMS,
                    lambda: [*DIST.glob("icortex*.whl"), *DIST.glob("icortex*.tar.gz")],
                ],
            ),
        ],
        targets=[SHA256SUMS],
...
def task_labext():
    ...
    def _copy_static():
        ...
        if pypi.exists():
            shutil.rmtree(pypi)
        copy_one(src, dest)
        ctx = "!!file-loader?name=pypi/icortex/[name].[ext]&context=.!../pypi/icortex"
        lines = [
            f"""export * as allJSONUrl from '{ctx}/all.json';""",
            f"""export * as icortexWheelUrl from '{ctx}/{dest.name}';""",
        ]
        pypi_ts.write_text("\n".join(lines))
...
def task_lite():
    ...
    def _wheels():
        ...
        wheel = sorted(DIST.glob("icortex-*.whl"))[-1]

        for old_wheel in LITE_PYPI.glob("icortex-*.whl"):
            old_wheel.unlink()
